...
SERVER=$(curl -X POST -H 'Content-type: application/json' -H 'Accept: application/json' -H 'Authorization: Bearer {!! $credential->access_token !!}' -d  "$(SERVER_DATA)" -X POST {{ $route('servers.create-no-credential') }})
...
echo '#####################################################################'
echo '#                                                                   '
echo '# Thank you for configuring your server with {{ $url('') }}!'
echo "# You can view your configured server at this URL {{ $route('spa', ['servers']) }}/$(echo $SERVER | jq -r '.id')"
echo '#                                                                   #'
echo '#####################################################################'
