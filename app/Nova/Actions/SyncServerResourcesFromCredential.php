...
class SyncServerResourcesFromCredential extends Action implements ShouldQueue
{
    use Batchable;
    ...
    public function handle(ActionFields $fields, Collection $models)
    {
        if ($this->batch()) {
            $this->batch()->add(
                $models->map(fn(Credential $model) => new FetchServersForCredential($model))
                    ->toArray()
            );
        } else {
            Bus::batch(
                $models->map(fn(Credential $model) => new FetchServersForCredential($model))->toArray()
            )
